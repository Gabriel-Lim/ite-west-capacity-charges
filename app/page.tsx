...
const EnergyModelingApp: React.FC = () => {
  ...
  // On mount, mark as client so we can render recharts
  useEffect(() => {
    setIsClient(true);
  }, []);
  ...
  // Identify min and max of effective demands (used for custom negative-case text)
  ...
  // When "Set Optimal Capacity" is clicked
  ...
  // Monthly charges with the current battery & contracted capacity
  const monthlyCharges = effectiveDemands.map((effDemand) =>
    calculateMonthlyCharge(effDemand, contractedCapacity)
  );
  const totalCharge = monthlyCharges.reduce((sum, charge) => sum + charge, 0);

  // Original charges without battery, contracted capacity fixed at 3,100 kW
  const originalCharges = maxDemands.map((demand) => calculateMonthlyCharge(demand, 3100));
  const totalOriginalCharge = originalCharges.reduce((sum, charge) => sum + charge, 0);

  // Net savings (could be negative if new scenario is worse)
  ...
  const netAmount = formatCurrency(Math.abs(savings)); // display as positive number if negative

  // Determine why it costs more if negative
  // - If contractedCapacity > maxEffectiveDemand => likely paying for unneeded capacity
  // - Else if contractedCapacity < minEffectiveDemand => likely incurring exceedance penalties
  let negativeReason =
  ...
  if (contractedCapacity > maxEffectiveDemand) {
    ...
    // This is basically the default text, but left here for clarity
    negativeReason =
      'the contracted capacity may be set too low relative to your effective demands, causing higher exceedance penalties.';
  }

  // Calculate monthly savings vs. original
  const monthlySavings = maxDemands.map((demand, index) => {
    ...
    return originalCharge - newCharge; // could be negative if new scenario is worse
  });
  ...
  // Handle dragging of the contracted capacity line in the chart
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isDragging && chartRef.current) {
        ...
        if (yPixel >= 0 && yPixel <= chartHeight) {
          // Map the Y coordinate to 0-4000 kW
          const yValue = (1 - yPixel / chartHeight) * 4000;
          // Round to nearest 10 kW
          const newCapacity = Math.round(yValue / 10) * 10;
          // Clamp between 1000 and 4000
  ...
  // Dynamically calculate chart width (90% of container width)
  const containerWidth = chartRef.current ? chartRef.current.offsetWidth * 0.9 : 700;

  return (
    <div className="p-6 font-sans max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Capacity Charge Modeling - ITE College West</h1>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <div className="flex-1">
          <label className="block text-sm font-medium text-gray-700">Contracted Capacity (kW)</label>
          <input
            type="number"
            min={1000}
            max={4000}
      ...
      {/* Explanation (shows up after clicking "Set Optimal Capacity") */}
      {explanation && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="p-4 bg-gray-50 rounded-md shadow-sm">
          ...
          <p className="text-2xl font-bold text-gray-800">{formatCurrency(totalCharge)}</p>
        </div>
        <div className="p-4 bg-gray-50 rounded-md shadow-sm">
          <p className="text-sm font-medium text-gray-500">Savings vs Original (8 months)</p>
      ...
      <div className="p-4 bg-white border border-gray-200 rounded-md shadow-sm mb-6">
        ...
        <div className="overflow-x-auto mb-3">
          <table className="min-w-full bg-white border border-gray-200 rounded-md shadow-sm">
            ...
            <tbody className="text-gray-700 text-sm">
              {months.map((month, index) => {
                const saving = monthlySavings[index]; // Could be negative or positive
      ...
      {/* Assumptions & Limitations and How It Works Cards (Side by Side) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="p-4 bg-white border border-gray-200 rounded-md shadow-sm">
          ...
          <ul className="list-disc list-inside space-y-2 text-sm text-gray-500">
            <li>The battery reduces maximum demand each month through peak shaving, assuming full availability and <strong>perfect</strong> prediction of peak usage.</li>
            ...
            <li>The original baseline scenario assumes no battery usage and a fixed contracted capacity of 3,100 kW, reflecting historical data from May to December 2024.</li>
        ...
        <div className="p-4 bg-white border border-gray-200 rounded-md shadow-sm">
          ...
          <div className="text-sm text-gray-500">
            <p className="mb-2">
              ...
              and accounting for a battery that reduces peak demand by up to its rated power capacity (e.g. 400 kW).
            </p>
